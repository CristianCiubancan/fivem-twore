...
export async function createBuilder(watch, baseOptions, environments, onBuild) {
    ...
    try {
        ...
        try {
            ...
            const wasmURL = new URL('../../node_modules/esbuild-wasm/esbuild.wasm', import.meta.url).href;
            await esbuildPkg.initialize({ wasmURL });
            useWasm = true;
        } catch (err) {
            console.error('esbuild is not installed and wasm fallback failed. Skipping build.');
            return;
        }
    }
    const outfiles = {};
    ...
    await Promise.all(environments.map(async ({ name, options }) => {
        ...
        if (useWasm) {
            ...
            try {
                ...
                console.warn(`esbuild context error for ${name}: ${err.message}. Falling back to WASM build.`);
                // Fallback to WebAssembly build
                try {
                    const mWasm = await import('esbuild-wasm');
                    esbuildPkg = mWasm;
                    const wasmURL = new URL('../../node_modules/esbuild-wasm/esbuild.wasm', import.meta.url).href;
                    await esbuildPkg.initialize({ wasmURL });
                    useWasm = true;
